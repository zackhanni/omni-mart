...
export default function Nav() {
  ...
  return (
    <nav className="sticky top-0 w-full z-20 bg-gradient-to-r from-indigo-950 to-purple-700 text-white">
      {/* top section nav */}
      <div className="flex flex-row justify-between items-center font-bold w-full px-4 sm:space-x-4">
        <Link href="/" className="translate-y-1 no-underline text-white">
          <Logo className="hidden sm:block" />
          <div>
            <p className="m-0 text-lg sm:hidden leading-none">OMNI</p>
            <p className="m-0 text-lg sm:hidden leading-none">MART</p>
          </div>
        </Link>
        <form className="flex py-3 md:min-w-[60%] px-4">
          <input
        ...
        <div className="flex items-center md:space-x-8">
          <Link
            href="orders"
            className="no-underline text-white hover:underline rounded-full items-center hidden md:flex flex-row"
          >
